...
//#if defined(WIN32) || defined(__CYGWIN__)
	...
	if(rc != MOSQ_ERR_SUCCESS) return BROKER_LIB_ERROR_BAD_ARGUMENT;
	int_db.config = &config;

    /* if port was passed in as a parameter, override port in all listeners */
    if (port > 0)
    {
        for (i = 0; i < config.listener_count; i++)
        {
            config.listeners[i].port = port;
        }
    }
	...
	if(rc != MOSQ_ERR_SUCCESS){
		...
		return BROKER_LIB_ERROR_DATABASE_OPEN;
	}
	...
	if(rc) return BROKER_LIB_ERROR_SECURITY_MODULE_INIT;
	rc = mosquitto_security_init(&int_db, false);
	if(rc) return BROKER_LIB_ERROR_SECURITY_INIT;
	...
	for(i=0; i<config.listener_count; i++){
		if(config.listeners[i].protocol == mp_mqtt){
			if(mqtt3_socket_listen(&config.listeners[i])){
				...
				return BROKER_LIB_ERROR_SOCKET_OPEN;
			}
			...
			if(!listensock){
				...
				return BROKER_LIB_ERROR_SOCKET_LISTEN;
			}
			for(j=0; j<config.listeners[i].sock_count; j++){
				if(config.listeners[i].socks[j] == INVALID_SOCKET){
				    ...
				    return BROKER_LIB_ERROR_SOCKET_INVALID;
				}
			...
			if(!config.listeners[i].ws_context){
				...
				return BROKER_LIB_ERROR_WEBSOCKET_CREATE;
			}
#endif
		}
	}
	...
	if(rc != MOSQ_ERR_SUCCESS) return BROKER_LIB_ERROR_PRIVILEGE_DROP;
